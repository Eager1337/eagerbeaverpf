...
function RevealLayer({ image, cursorX, cursorY }: { image: string; cursorX: number; cursorY: number }) {
  ...
  useEffect(() => {
    ...
    (r.style as unknown as { webkitMaskImage: string }).webkitMaskImage = `url(${url})`;
    r.style.maskSize = "100% 100%";
    (r.style as unknown as { webkitMaskSize: string }).webkitMaskSize = "100% 100%";
  }, [cursorX, cursorY]);
      ...
      <section className="relative w-full overflow-hidden h-screen bg-black" style={{ height: "100dvh" }}>
        ...
        <nav className="absolute top-0 left-0 right-0 z-50 flex items-center justify-between px-5 sm:px-8 md:px-10 py-5">
          <div className="flex items-center gap-2 text-white">
            ...
            <span className="font-semibold text-lg">Lithos</span>
          </div>
          <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full px-2 py-2 items-center gap-1">
            ...
            {["Field Guides", "Geology", "Plans", "Live Tour"].map((l) => (
              <button key={l} className="text-white/80 hover:bg-white/20 hover:text-white transition-colors px-4 py-1.5 rounded-full text-sm font-medium">
                {l}
              </button>
            ))}
          </div>
          ...
          <button aria-label="Open menu" className="md:hidden text-white">
